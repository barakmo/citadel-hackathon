...
import { GrantFlowDoneDto } from '../../dto/grant-flow-done.dto';
import { ActiveFlow, FlowStatus } from '../../db/entity/active-flows';
...
export class UserPermissionService {
  ...
  async assignPermissionToUser(
    ...
    if (appId && permission.app.id !== appId) {
      throw new NotFoundException(
        'Permission not found in the specified application',
      );
    }

    // Check if user already has this permission
    const hasPermission = user.allowedPermissions.some(
      (p) => p.id === permissionId,
    );
    if (!hasPermission) {
  ...
  async getUserPermissions(
    ...
    const appPermissions = user.allowedPermissions.filter(
      (p) => p.app.id === appId,
    );

    return appPermissions.map((permission) =>
      PermissionDto.fromPermission(permission),
    );
  }
  ...
  async hasPermission(
    ...
    return user.allowedPermissions.some(
      (p) => p.id === permissionId && p.app.id === appId,
    );
  }
  ...
  async getUserPermissionsByResource(
    ...
    const resourcePermissions = user.allowedPermissions.filter(
      (p) => p.resource && p.resource.id === resourceId,
    );
  ...
  async getUserPermissionsGroupedByResource(
    ...
    for (const permission of user.allowedPermissions) {
      ...
      resourceMap
        .get(resourceId)
        .permissions.push(PermissionDto.fromPermission(permission));
    }
  ...
  async assignPermissionToTemplate(
    ...
    const hasPermission = template.permissions.some(
      (p) => p.id === permissionId,
    );
    if (!hasPermission) {
  ...
  async removePermissionFromTemplate(
    ...
    // Filter out the permission to remove
    template.permissions = template.permissions.filter(
      (p) => p.id !== permissionId,
    );
    await this.crud.update<AccessTemplate>(template);
  }
  ...
  async getUserAccessTemplates(userId: string): Promise<AccessTemplateDto[]> {
    ...
    return user.templates.map((template) =>
      AccessTemplateDto.fromAccessTemplate(template),
    );
  }

  // Helper methods
  private async getUserWithPermissions(userId: string): Promise<User> {
    const user = await this.crud.readWithRelations<User>('User', { userId }, [
      'allowedPermissions',
      'allowedPermissions.app',
  ...
  private async getPermissionById(permissionId: number): Promise<Permission> {
    const permission = await this.crud.readWithRelations<Permission>(
      'Permission',
      { id: permissionId },
      ['app', 'resource'],
    );
  ...
  private async getTemplateById(templateId: number): Promise<AccessTemplate> {
    const template = await this.crud.read<AccessTemplate>({
      id: templateId,
    } as AccessTemplate);
    if (!template) {
  ...
  private async getTemplateWithPermissions(
    templateId: number,
  ): Promise<AccessTemplate> {
    const template = await this.crud.readWithRelations<AccessTemplate>(
      'AccessTemplate',
      { id: templateId },
      ['permissions', 'permissions.app', 'permissions.resource'],
    );
    if (!template) {
      throw new NotFoundException('Access Template not found');
    }
  ...
  private async getGrantFlowById(grantFlowId: number): Promise<GrantFlow> {
    const grantFlow = await this.crud.read<GrantFlow>({
      id: grantFlowId,
    } as GrantFlow);
    if (!grantFlow) {
  ...
  async updateFlow(req: GrantFlowDoneDto) {
    const activeFlow = await ActiveFlow.findOne({
      where: {
        flowId: req.flowExecutionId,
      },
    });
    if (!activeFlow) {
      throw new NotFoundException('Flow not found');
    }
    activeFlow.flowStatus = req.allowed
      ? FlowStatus.APPROVED
      : FlowStatus.DENIED;
